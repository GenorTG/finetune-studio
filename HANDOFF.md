...
| **Per-commit build versioning** | `97a2c0c`: repo-root `VERSION` = MAJOR.MINOR.PATCH.BUILD; a versioned `commit-msg` hook (`make hooks`, `core.hooksPath`) auto-bumps BUILD on EVERY commit — the UI chip ("EARLY BETA release · v0.1.0.N") and `GET /api/system/version` (version+channel+git_commit) always pin the exact deployed code. Escape hatch: commit message containing the literal no-bump token; VERSION-only commits don't bump (loop guard). Hook verified live: 0.1.0.1 → 0.1.0.2 on the wizard fix. 4 tests in `tests/test_version.py` (format, endpoint, hook behavior in a sandbox repo, executability). |
