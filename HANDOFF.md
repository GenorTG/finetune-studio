...
| QABUG-008 (dark/light toggle stays dark) | ✅ SHIPPED `b6ef4f6` + **live-verified** on fan-dragon pid `1686790` | toggle button + `[data-theme]` palette CSS + `localStorage.fts-theme` wiring + pre-paint inline `<script>` in `<head>` all working; 3 screenshots confirm: (1) initial dark mode, (2) click → light mode swap (palette flips: dark bg → off-white bg, bright green text → dark teal text, moon icon → sun icon), (3) reload → dark mode preserved (localStorage persistence verified) |
...
| QABUG-012 (bench page JSON dump) | ✅ SHIPPED `b6ef4f6` — **server-renders** per-case detail via `_case_results.html` partial + `latest_cases` context var; curl-strip confirmed body contains `Latest case results total: 10 judged: 10 passed: 8 partial: 2 failed: 0 pass_rate: 80.0` + all 10 case rows with verdicts + judge_reasoning. The "COMPUTING SCORES" sprite is the **idle animation** (green ghost bars + caption in `sprites.js:640`), not a stuck loading state — it listens for `fts:bench-progress` events to show `SCORE 80%` after the user clicks "Run a benchmark". |
| QABUG-013 (testing page JSON dump) | ✅ SHIPPED `b6ef4f6` — **client-renders** per-case detail via `renderResults(scores, cases)` JS function in `project_testing.html` after the user clicks "Run" + suite completes; `recent_suite_runs` context var server-renders the suite-run history table (top 5 most recent runs). `/api/testing/run-suite` returns full v2-schema payload (`scores` + per-case `results` array). The sprite idle is the same as the bench page. |
...
| **QABUG-015 (RETRO: bench/test page "stuck in COMPUTING SCORES")** | ❌ **RETRACTED — was a misread on my end**. The "COMPUTING SCORES" text is the **idle sprite animation** (green ghost bars + caption in `src/finetune_studio/webui/static/js/sprites.js:640`), not a stuck loading state. The sprite listens for `fts:bench-progress` events to display `SCORE 80%` after a real run completes. Both pages (bench + testing) DO render per-case detail correctly: bench page via server-rendered `_case_results.html` partial (curl-strip confirmed), testing page via client-rendered `renderResults(scores, cases)` JS function. No bug to fix. |
