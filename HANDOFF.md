...
| Tests | Focused export suite: **66 passed** locally; changed export files pass direct Ruff |

## Next steps
1. Fan-dragon: `cd /home/genortg/finetune-studio && git pull --ff-only && systemctl --user restart finetune-studio`.
2. Browser: Export GGUF `q8_0` on a merged run — expect non-empty artifact + registry row.
3. Optional GPTQ: `uv pip install --python .venv/bin/python -e '.[gptq]'` then restart; Export GPTQ when checkbox enabled.
...
- Verified 2026-09-15: focused export suite completed with `66 passed, 2 warnings in 5.20s`; direct Ruff on changed export files returned `All checks passed!`. Full-repo Ruff remains baseline-dirty outside this change.
