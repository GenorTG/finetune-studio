...
## State (verified 2026-09-20 ~20:22 CEST · fan-dragon: `GET /api/system/version` = live build, last seen `0.1.0.8` @ `266ffc1`, service active)
...
| **File browser v2 (all 10 missing capabilities)** | `e71645b`+`7e493c7`: folder chips row (create/rename/delete + drag-drop move — raw→user-folder membership is now a legal organizing label, bytes immutable), pagination + page-size, Columns menu (persisted), thumbnails, shift-click range select, click-to-filter tag pills, bulk engine on ONE `POST /files/bulk` (delete/restore/move/reparse/tag-add/tag-remove, per-file isolation), ⬇ Zip export (`download-zip`, 2 GiB cap, collision-safe names), ℹ️ usage chain (`/files/{fid}/usage`: file→source→pairs→datasets→runs+RAG), parsed-content search (`/files/search-content`), trash restore-all + retention selector. Global `[hidden]{display:none!important}` fix (.btn/.pill display was overriding the attribute). 11 tests `tests/test_file_browser_v2.py`; ALL features click-verified live on fb05a690 (56 seeded files): folder create→rename→filter, drag-drop move, zip download event, content search 25 hits→editor modal, usage modal, shift-select, bulk tag+pill filter, pagination 1/2, columns hide, trash restore. |
| **Visual quality sweep (every page, probe-audited)** | `e902611`→`dc8f3b3` (css v38–v42): browser-injected audit probe (overflow past viewport + clipped cells + <10px text + WCAG contrast vs composited bg) run on ALL 15 routes × dark/light × 1270px/320px. Fixed: file-table actions never clip (fixed rem cols, table-scroll absorbs — last button verified reachable after scroll); pipeline badges wrap; run/model/suite names wrap + title tooltips instead of silent 300–400px truncation (overview/models/training/benchmarks incl. recent-scores); `--on-accent` token + aliases for 13 legacy token names (`--muted`/`--danger`/`--accent-red|rose|green|amber|cyan|violet`/`--bg-deep`/`--surface`) — templates referenced undefined vars so LIGHT THEME silently got dark-neon fallbacks (1.3:1 text); bench actions cell stacks its two forms (Train-set eval was 146px past the cell); mobile session-bar cluster box-sizing (16px viewport spill) + overview tables wrapped in table-scroll (45 action buttons were unreachable at 320px); wizard tabs wrap; tree labels 9→10.5px. Final probe: 0 real overflows, 0 clips-without-tooltip, 0 sub-3:1, 0 <10px text in both themes at both widths. |
| **Artifact naming scheme** | `7777981`: new `finetune_studio/naming.py` — every model/dataset label reads `<Project> · <Base> [· vN label] [· Kind] [· QUANT] [· abliterated]`. Root cause fixed: the registry's raw-sqlite project lookup pointed at a DB path that doesn't exist on the deployed box, so run exports rendered as bare hash dirs ("6f64c46a/merged", running model "_merged"). Model pickers, `/api/inference/status.model_display`, the loaded-model header, and dataset registry names all flow through naming (project name · base model · pinned version · kind · quant · abliterated). 10 tests in `tests/test_naming.py`. |
| **File workbench (manual file work for BOTH flows)** | `8efaa80`+`e7e9f7f`+`266ffc1`: the data-page browser is now the hub — tabbed modal (original viewer + **built-in parsed-text editor**), pipeline badges (parsed / prep·Nch / rag), sortable columns, ⚡ Send-to-Data-Prep (single + bulk), ↻ re-parse. `PUT /files/{fid}/parsed` writes a collision-proof `<raw>.parsed.md` override and re-chunks the prep source **by content sha256**, so hand-edits feed Q&A/training AND the next RAG build while raw bytes stay immutable; RAG page gains **⚡ Quick index** (`POST /rag/quick` = promote-missing + build, one click). Live-verified on fb05a690: save → "re-chunked to 1 chunks", re-parse → original restored, quick index → 3 docs/3 chunks, badges green, chip `v0.1.0.8`. 8 tests in `tests/test_file_workbench.py`. |
