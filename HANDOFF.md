...
## State (verified 2026-09-14 16:00)
| Area | Status |
|------|--------|
| Git | `main` — benchmarks compare tab (this commit) |
| Benchmarks compare (`/projects/{pid}/benchmarks`) | ✅ Done: tabs Recent scores + Compare two runs; pickers + GET `/api/benchmarks/projects/{pid}/compare`; per-suite Δ (B−A). Note: claimed `POST /api/comparison/load` does **not** exist — wired to the real compare route in `routes/benchmarks.py` |
| Testing suite dropdown | ✅ Done (`7ec9ad4` + docs `5fa82e1`) |
...
## Next steps
1. Pull + restart on fan-dragon; smoke `http://fan-dragon:7860/projects/b08426e3/benchmarks` (both tabs, Run comparison with two scored runs)
2. **UI audit Phase 2 remaining** — models expand-row + Open in inference, RAG docs-indexed panel, export expand-row, settings log tail (`/api/logs`)
