...
| Source-disjoint quality gate | `scripts/build_source_disjoint_split.py` created **194 train pairs / 28 sources** and **52 held-out pairs / 7 unseen sources**. Run `f4f627af` trained 6 epochs (`final_loss=0.1273`); explicit Q8 benchmark `88c423d9` is **0 pass / 12 partial / 40 fail = 11.5% weighted**, with **52/52 valid transcripts and 0 audit disagreements**. This is the honest generalization result: the model memorizes source facts and does not transfer to unseen sources. |
...
| Last code | `32d8dd4` adds source-disjoint split generation/tests; `2518377` lets benchmarks select and record the exact export; `4364886` preserves source provenance; `fa84208` moves Agent inference off the event loop. |
