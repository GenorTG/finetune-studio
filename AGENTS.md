...
- 2026-09-20 **Per-commit versioning:** run `make hooks` once per clone; the commit-msg hook bumps `VERSION` (BUILD segment) on every commit and the UI chip + `/api/system/version` show it. Never put the literal no-bump token in a commit message unless you mean it — the first versioning commit self-skipped the bump that way (97a2c0c).
