...
- 2026-09-15 "v3 merged model gives 'Qwen3Attention' object has no attribute 'apply_qkv'" — QABUG-014 / QABUG-014-runtime. Importing `unsloth` before transformers is **not** enough: the checkpoint must be loaded via `FastQwen3Model.from_pretrained(..., load_in_4bit=True)` (or `FastLanguageModel` for non-Qwen3) so unsloth_zoo patches bind before the first forward. Vanilla `AutoModelForCausalLM.from_pretrained` on a Unsloth-trained merge still raises `apply_qkv` at chat time. Also: `POST /api/testing/load` and `/api/chat-v2/load` must accept both `model_path` and `path` keys.
